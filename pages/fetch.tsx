...
const Fetch: NextPage = () => {
  ...
  const loadMovies = async (page: number) => {
    try {
      ...
      const response = await fetch(
        `${BASE_URL}${MOVIE_URL}&page=${page}${API_KEY}`
      );
  ...
  const loadData = React.useCallback(async (page: number) => {
    const movies = page ? await loadMovies(page) : undefined;
    setData(movies);
  }, []);
  ...
  return (
    <Template>
      ...
      {/* Loading */}
      {isValidating && <h1 style={{ textAlign: "center" }}>Loading...</h1>}

      {/* Error */}
      {error && <h1>Some Errors shows</h1>}
